<template>
  <div>
    <button class="trigger" @click="isOpen = true" aria-label="Open levels">
      <!-- Planet/rocket/wheat icon -->
      <span class="icon" aria-hidden="true">🌾</span>
    </button>

    <transition name="fade-up">
      <div v-if="isOpen" class="overlay" @click.self="close">
        <div class="panel" role="dialog" aria-modal="true" aria-label="Levels selector">
          ...
          <div class="panel-body">
            <div class="seasons-grid">
              ...
              >
                <div class="season-media" :style="{ backgroundImage: `url(${seasonImage[s.id] || ''})` }"></div>
                <div class="season-title">{{ s.title }}</div>
                <div class="season-meta">Levels {{ s.range }}</div>
                <div class="season-progress">{{ s.completedCount }} / {{ s.levels.length }} completed</div>
              </button>
            </div>
          </div>
...
.overlay {
  position: fixed;
  inset: 0;
  background: radial-gradient(120% 120% at 50% 0%, rgba(10, 12, 24, 0.45) 0%, rgba(6, 8, 20, 0.45) 60%, rgba(4, 6, 14, 0.45) 100%);
...
.season-card {
  ...
  background-image:
    radial-gradient(2px 2px at 8% 20%, rgba(170, 200, 255, 0.9) 40%, transparent 41%),
    radial-gradient(1px 1px at 30% 70%, rgba(200, 220, 255, 0.85) 40%, transparent 41%),
    radial-gradient(1.5px 1.5px at 65% 35%, rgba(150, 190, 255, 0.8) 40%, transparent 41%),
    radial-gradient(1px 1px at 85% 55%, rgba(180, 210, 255, 0.75) 40%, transparent 41%),
    radial-gradient(0.8px 0.8px at 45% 15%, rgba(255, 255, 255, 0.7) 40%, transparent 41%);
  background-repeat: repeat;
  background-size: 140px 90px, 160px 100px, 180px 120px, 200px 140px, 220px 160px;
  background-blend-mode: screen;
  box-shadow: 0 0 10px rgba(0, 153, 255, 0.18), inset 0 0 6px rgba(0, 153, 255, 0.1);
...
.season-card:hover {
  ...
  box-shadow: 0 0 16px rgba(0, 180, 255, 0.3), 0 0 2px rgba(0, 180, 255, 0.35) inset;
  transform: translateY(-1px);
}
.season-card:focus-visible {
  outline: none;
  border-color: rgba(0, 200, 255, 0.9);
  box-shadow: 0 0 18px rgba(0, 200, 255, 0.4), 0 0 3px rgba(0, 200, 255, 0.45) inset;
}

.season-title { font-weight: 800; letter-spacing: 0.5px; margin-bottom: 4px; }
.season-meta { color: #9fb3d9; font-size: 14px; }
.season-progress { color: #7fa3ff; font-size: 12px; margin-top: 6px; }
...
.fade-up-enter-active { transition: opacity 220ms ease, transform 220ms ease; }
.fade-up-leave-to { opacity: 0; transform: translateY(8px); }
.fade-up-leave-active { transition: opacity 160ms ease, transform 160ms ease; }
